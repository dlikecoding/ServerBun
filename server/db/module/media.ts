...
// SQL Queries
const Sql = {
  UPDATE: `UPDATE Media SET HashCode = ? WHERE media_id = ?`,
...
const updateHash = async (media_id: number, HashCode: string | null) => {
  await poolPromise.execute(Sql.UPDATE, [media_id, HashCode]);
...
export { updateHash, deleteMedia, fetchAllMedia, findMediaById, markHidden, markFavorite, markDeleted };
