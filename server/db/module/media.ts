...
const Sql = {
  ...
  LOAD_MEDIA_THUMB: `SELECT md.media_id, md.SourceFile, md.ThumbPath, md.FileType FROM ImportMedias im LEFT JOIN Media md ON im.import_id = md.media_id`,
...
export const mediaWoThumb = async () => {
  const [rows] = await poolPromise.execute(Sql.LOAD_MEDIA_THUMB);
  return rows as any;
};
