...
const createAdminOrUsers = async (user_name: string, user_email: string) => {
  const result = await sql.begin(async (tx) => {
    const [isAdminExist] = await tx`SELECT reg_user_id FROM multi_schema."RegisteredUser" LIMIT 1`;
    const userId = Bun.randomUUIDv7();

    const [serverId] = await sql`SELECT * FROM multi_schema."ServerSystem" LIMIT 1`;

    await tx`INSERT INTO multi_schema."RegisteredUser" (reg_user_id, user_name, user_email, role_type, status, "ServerSystem") 
              VALUES (${userId}, ${user_name}, ${user_email}, ${!isAdminExist ? 'admin' : 'user'}, ${!isAdminExist}, ${serverId.system_id})`;
    return userId;
  });
...
const createPasskey = async (
  ...
  try {
    const [result] = await sql`INSERT INTO multi_schema."Passkeys" ( cred_id,  cred_public_key,  "RegisteredUser",  counter,  registered_device,  backup_eligible,  transports) 
...
const findRegUser = async (user_email: string): Promise<UserType> => {
  const [user] = await sql`SELECT reg_user_id, user_email, status, role_type FROM multi_schema."RegisteredUser" WHERE user_email = ${user_email} LIMIT 1`;
  return user;
};

const userPassKeyByEmail = async (user_email: string) => {
  const [rows] =
    await sql`SELECT reg.reg_user_id, reg.user_email, reg.user_name, reg.role_type, reg.status, pks.cred_id, pks.cred_public_key, pks.counter, pks.transports FROM multi_schema."RegisteredUser" reg 
...
const countSuspendedUsers = async () => {
  const [result] = await sql`SELECT COUNT(reg_user_id) AS count_users FROM multi_schema."RegisteredUser" WHERE status = false`;
  return result.count_users;
};

const fetchAllUsers = async () => {
  return await sql`SELECT * FROM multi_schema."RegisteredUser" reg WHERE role_type = 'user'`;
};

const updateAccountStatus = async (userEmail: string) => {
  const [result] = await sql`UPDATE multi_schema."RegisteredUser" SET status = NOT status WHERE user_email = ${userEmail} RETURNING reg_user_id`;
  return result;
};
