...
CREATE OR REPLACE FUNCTION multi_schema.auto_insert_duplicate_if_found_hash()
RETURNS trigger AS $$
BEGIN
	DECLARE dup_media SERIAL;
	
    IF EXISTS (
        SELECT 1 FROM "multi_schema"."Media"
        WHERE hash_code = NEW.hash_code
    ) THEN 
        INSERT INTO multi_schema."Duplicate"(media) VALUES(NEW.media_id)
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION "multi_schema".auto_insert_duplicate_if_found_hash();
