...
BEGIN
    ...
    IF count_duplicate = 1 THEN
        DELETE FROM multi_schema."Duplicate"
        WHERE hash_code = OLD.hash_code;
    END IF;
...
-- -- Trigger function in Media table check if the hash code exist (BEFORE UPDATE hash code), if so insert to Duplicate table
-- CREATE OR REPLACE FUNCTION multi_schema.insert_or_track_duplicate()
-- RETURNS trigger AS $$

-- DECLARE existing_media_id INTEGER;

-- BEGIN
--     SELECT media_id INTO existing_media_id FROM multi_schema."Media"
--     WHERE hash_code = NEW.hash_code AND media_id <> NEW.media_id
--     LIMIT 1;

--     IF FOUND THEN
--         INSERT INTO multi_schema."Duplicate"(media)
--         VALUES (existing_media_id), (NEW.media_id)
--         ON CONFLICT DO NOTHING;  -- Avoid duplicate pairs
--     END IF;
...
-- CREATE OR REPLACE TRIGGER insert_to_duplicate_table_before_update
-- BEFORE UPDATE ON multi_schema."Media"
-- FOR EACH ROW
-- EXECUTE FUNCTION multi_schema.insert_or_track_duplicate();
